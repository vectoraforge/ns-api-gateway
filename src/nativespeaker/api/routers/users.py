...
from nativespeaker.api.schemas.auth import LinkedIdentity, MeResponse, Profile

# Router-level auth protects an endpoint added later whose own Depends is forgotten; the same callable runs once.
router = APIRouter(tags=["users"], dependencies=[Depends(get_identity)])
...
async def me(response: Response,
    ...
    purchase_tokens = await purchases.read_tokens(linked.user.id)
    # `no-store` rather than `no-cache`: the tokens are secrets, and a revalidatable copy is a copy.
    response.headers["Cache-Control"] = "no-store"
    return MeResponse(profile=Profile(email=linked.user.email,
                                      display_name=linked.user.display_name),
                      identity_provider=linked.identity.provider,
