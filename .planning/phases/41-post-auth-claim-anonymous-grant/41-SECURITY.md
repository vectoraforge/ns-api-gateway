...
| T-41-06 | Tampering | destroying Phase 42's bit1 state | high | mitigate | The write carries the queried bit1 rather than fabricating one (`services/auth.py:232`: `bit1=state.bit1`); `test_devicecheck_adapter.py::TestTheBit1CarryForward`. Scope is fabrication only: losing a value another request wrote inside the read-to-write window is T-41-31 | closed |
...
| T-41-31 | Tampering | a concurrent claim's bit write lost on one device | medium | accept | Both claim routes read Apple's pair before their transaction opens and write it back with one member replaced (`services/auth.py:206`→`:232`, `:277`→`:298`), and they run against different `core.users` rows, so no grant lock serialises them. Apple's `update_two_bits` has no compare-and-swap and `SHARED-INVARIANTS.md` forbids a distributed lock, lease or multi-phase commit, so no in-bounds fix exists; the loss is one free-tier grant. Recorded as AR-41-03 | closed |
...
| AR-41-03 | T-41-31 | Two claims racing on one physical device from two accounts each carry the other bit forward from a read taken before its own transaction, so the later write reopens the earlier writer's slot and one extra free grant becomes claimable. Apple offers no compare-and-swap, and the alternatives are forbidden: `SHARED-INVARIANTS.md` bans "distributed lock, lease, or multi-phase-commit machinery" and "device-fingerprint or device-check component in any rate-limit key, in any form". Accepted: the loss is one anonymous-tier grant on a sub-$5 product. `devicecheck.py::write_bits_with_retry` carries the contract | Phase 41 review WR-21 | 2026-09-09 |
...
| 2026-09-09 | 32 | 31 | 1 (medium, non-blocking) | code-review fix pass, WR-21: T-41-31 added and accepted as AR-41-03; T-41-06 rescoped to fabrication |
