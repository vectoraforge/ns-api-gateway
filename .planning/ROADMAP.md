...
3. Every SQL comparison against the current time uses `now()`; every Python read of the current time is a `datetime.now(UTC)` call at the point of use, with at most a small pure helper taking the datetime it computes from — **met in spirit, deviation named, 2026-09-12. The comparison is database-side, but the function is `clock_timestamp()` and not `now()`.** D-01 made that substitution, and the reason was measured rather than argued: PostgreSQL's `now()` is `transaction_timestamp()`, and `tests/e2e/conftest.py` holds one outer transaction open per test and joins every app session to it with `create_savepoint`, so a grant a test seeds is invisible to `now()` — RESEARCH Finding 1 measured 11 of 41 quota tests turning red under it. The two SQL sites are `_effective_grants_statement` in `crud/grants.py` (both bounds) and `ChallengesDB.claim` in `crud/challenges.py`, where the expiry predicate and the `claimed_at` write are one evaluation in one statement; `consume` stamps `consumed_at` from the same clock. `grep -rn 'func.now()' src/` prints nothing. **The Python half is met as written:** 19 reads, one per method, each the first statement of the method that stamps from it — `charge`, `read_entitlement`, `restore`, `ingest`, `insert_account`, `flip_provider`, both `activate_*_grant` writers, the seven `SubscriptionsDB` writers, `ChallengesDB.issue`, `PlayDeveloperSubscriptions.read` and `.read_for_restore`, and `AppStoreNotifications.verify_transaction`. Two further reads in `auth/devicecheck.py` predate the phase and are unchanged.
