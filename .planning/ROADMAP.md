...
5. `.venv/bin/pytest -q -m ''`, `-m e2e` and `-m schema` all exit 0 — **NOT MET, 2026-09-12.** Measured in plan 47-08's own run, never copied. `-m schema` is **291 passed, exit 0**. `-m e2e` is **360 passed, 1 failed, exit 1**. `-m ''` is **2571 passed, 2 failed, exit 1**, 2573 collected against a 2570 pre-phase baseline. **Two distinct failures, neither of them repaired here, because this plan writes no source.** The first is **pre-existing and not this phase's**: `tests/e2e/test_restore_subscription.py::…::test_the_four_arms_answer_bodies_equal_to_each_other_and_write_nothing` expects log event `proof_rejected` while the code emits `purchase_proof_rejected`; two executors measured it at HEAD `1a3273d` before the phase began, so any "e2e exits 0" criterion was unreachable from the start. The second **is this phase's own, and it is a new defect**: `tests/unit/test_subscription_attribution.py:964` builds an open term one minute past a `NOW` read at module import, so a run longer than a minute between collection and that case closes the term and `ingest` raises `InternalError`. It passes alone in 0.03 s and fails under the 131-second `-m ''` run; a throwaway plugin sleeping 65 s after collection reproduces it at the same raise site. Plan 47-05 introduced it. **Both are logged in `.planning/WINDOWS.md` and in the phase's `deferred-items.md`; closing them is a gap-closure plan's work, not this record's.**
...
| 47. Stop threading an evaluation instant through the layers | v2.0 | 8/8 | Executed — criterion 5 unmet | 2026-09-12 |
