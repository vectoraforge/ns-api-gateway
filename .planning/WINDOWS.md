...
| 33 | 48 | deviation | src/nativespeaker/api/services/auth.py |  | D-07 gives create-user the token dependency only, so no router-level declaration resolves that caller any more. Planning found the consequence: a caller holding an active row presents a challenge bound to that row, which the completion path could no longer confirm, so ChallengesDB.verify_binding would answer 409 challenge_required where the route answers 409 identity_already_linked today. The developer chose the option recorded in 48-01-SUMMARY.md - option B, AuthService.complete resolves the caller itself before the completion sequence. ROADMAP criterion 3 was amended on 2026-09-16 at plan time to name AuthService._complete beside crud/challenges.py, because _complete serves create-user, which has no row, and the three claim routes, which have one. | waived | Accepted under option B rather than fixed. Amended on 2026-09-16 after code review WR-01, which measured the wire: the earlier claim that every wire outcome is preserved was wrong. On POST /auth/challenge and POST /auth/create-user, a bad body or an unknown operation is now refused with 400 invalid_request or 422 validation_error before the account state is read. Before this phase the router-level get_identity read the account state first, so the same request from a historical row or a blocked user answered 403 account_unavailable. Both answers are refusals, and no accepted request changes. Every other outcome is preserved. The completion path issues one identity statement where the router-level dependency issued it before, so create-user's query count does not move. AuthService.complete resolves the caller at services/auth.py:81, before the claim, so the three rejections still refuse that caller before the challenge is spent, and D-07's accepted cost of one spent challenge does not ship. Measured at the phase gate in plan 48-08: tests/e2e/test_create_user.py answers 22 passed, including TestCompletionRejectsAnAlreadyLinkedCaller::test_an_active_linked_identity_is_rejected_at_completion, which asserts 409 identity_already_linked. | 2026-09-16T22:46:00.665Z | 2026-09-16T22:46:00.665Z |
...
[
  ...
  },
  {
    ...
    "status": "fixed",
    "reason": "Fixed at the root by the orchestrator in 5ad4fbd: the control now computes its open term at call time instead of dating it from the module-import NOW. Re-measured by plan 47-08 at HEAD 5ad4fbd - .venv/bin/pytest -q -m '' reports 1 failed / 2572 passed, and the one remaining failure is the unrelated pre-existing restore case.",
    "recorded_at": "2026-09-12T08:04:34.354Z",
    "resolved_at": "2026-09-12T08:20:17.143Z"
  },
  {
    "id": 32,
    "kind": "lint-warning",
  ...
  },
  {
    ...
    "status": "waived",
    "reason": "Accepted under option B rather than fixed. Amended on 2026-09-16 after code review WR-01, which measured the wire: the earlier claim that every wire outcome is preserved was wrong. On POST /auth/challenge and POST /auth/create-user, a bad body or an unknown operation is now refused with 400 invalid_request or 422 validation_error before the account state is read. Before this phase the router-level get_identity read the account state first, so the same request from a historical row or a blocked user answered 403 account_unavailable. Both answers are refusals, and no accepted request changes. Every other outcome is preserved. The completion path issues one identity statement where the router-level dependency issued it before, so create-user's query count does not move. AuthService.complete resolves the caller at services/auth.py:81, before the claim, so the three rejections still refuse that caller before the challenge is spent, and D-07's accepted cost of one spent challenge does not ship. Measured at the phase gate in plan 48-08: tests/e2e/test_create_user.py answers 22 passed, including TestCompletionRejectsAnAlreadyLinkedCaller::test_an_active_linked_identity_is_rejected_at_completion, which asserts 409 identity_already_linked.",
