...
the barrier really verifies, one identity query against the live database, and the response a client
actually receives.
...
async def barrier_client(_app_lifespan, stub_verifier):
...
class TestOutcomeFourLinkedAndActive:
    ...
    async def test_a_linked_active_identity_is_admitted(self, barrier_client, _db_transaction):
        subject = "admitted-subject"
        await seed_identity(_db_transaction, issuer=TEST_ISSUER, subject=subject)

        response = await barrier_client.get("/chats", headers=_auth(subject))

        assert response.status_code == 200

    async def test_the_handler_sees_the_resolved_user(self, barrier_client, _db_transaction):
        """The strongest form of "the handler sees the context".

        `list_chats` reads `identity.user.id` off the barrier's context and nothing else, so a chat
        owned by the seeded user comes back only if the id the handler used is the one the barrier
    ...
    async def test_another_subject_sees_none_of_it(self, barrier_client, _db_transaction):
        ...
        response = await barrier_client.get("/chats", headers=_auth("stranger-2"))

        assert response.status_code == 200
        assert response.json() == []

    async def test_an_anonymous_provider_is_admitted_too(self, barrier_client, _db_transaction):
...
class TestOutcomeOnePrimeUnlinked:
    ...
    async def test_an_unlinked_subject_is_refused(self, barrier_client):
        response = await barrier_client.get("/chats", headers=_auth("never-linked"))
        assert response.status_code == 403
        assert response.json() == {"code": "preauth_identity_not_allowed"}

    @pytest.mark.parametrize("path", ["/", "/examples?lang=en", "/chats"])
    async def test_no_authenticated_route_is_pre_auth_callable(self, barrier_client, path):
        """Only the two `create-user` phases may ever be, and neither exists yet."""
        response = await barrier_client.get(path, headers=_auth("never-linked"))
        assert response.status_code == 403
        assert response.json() == {"code": "preauth_identity_not_allowed"}
...
class TestOutcomesTwoAndThreeAreIndistinguishable:
    ...
    async def test_a_historical_identity_is_refused(self, barrier_client, _db_transaction):
    ...
    async def test_a_blocked_user_is_refused(self, barrier_client, _db_transaction):
    ...
    async def test_the_two_responses_are_identical_in_status_body_and_headers(
        ...
        historical = await barrier_client.get("/chats", headers=_auth("historical-pair"))
        blocked = await barrier_client.get("/chats", headers=_auth("blocked-pair"))
    ...
    async def test_neither_is_distinguishable_from_the_other_by_a_retired_pre_auth_answer(
        ...
        response = await barrier_client.get("/chats", headers=_auth("retired-not-fresh"))
...
class TestAdmissionPhasePrecedesAuth:
    """§4.1: route/method mismatch is decided before the barrier has anything to say."""

    async def test_a_wrong_method_returns_405_not_401(self, barrier_client):
        """`POST /` matches the path but not the method, so the router owns the answer."""
        response = await barrier_client.post("/")
        assert response.status_code == 405
        assert response.json() == {"code": "method_not_allowed"}

    async def test_the_405_holds_even_with_a_valid_credential(self, barrier_client,
                                                              _db_transaction):
        await seed_identity(_db_transaction, issuer=TEST_ISSUER, subject="wrong-method")
        response = await barrier_client.post("/", headers=_auth("wrong-method"))
        assert response.status_code == 405

    async def test_an_unknown_path_returns_404_not_401(self, barrier_client):
        response = await barrier_client.get("/no-such-path")
...
class TestVerificationPrecedesResolution:
    ...
    async def test_an_unverifiable_token_is_auth_required_not_preauth(self, barrier_client):
        response = await barrier_client.get("/chats",
                                            headers={"Authorization": "Bearer not.a.jwt"})
        assert response.status_code == 401
        assert response.json() == {"code": "auth_required"}

    async def test_a_wrong_issuer_is_auth_required(self, barrier_client, _db_transaction):
    ...
    async def test_an_expired_token_is_auth_required(self, barrier_client, _db_transaction):
