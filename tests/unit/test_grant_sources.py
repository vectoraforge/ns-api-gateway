"""ANONGRANT-01's single-writer claim, as a walk over `src/` rather than as a sentence.
The literal `anonymous_device_grant` is written from exactly one site, the crud activation
writer, and the free-grant membership is one named constant rather than a repeated pair.
"""
import ast
...
def _construction_sites(source: str) -> list[int]:
    """Line numbers of every `AccessGrant(...)` built with the anonymous device-grant source."""
    found = []
    for node in ast.walk(ast.parse(source)):
        ...
        for keyword in node.keywords:
            if keyword.arg == "source" and _names_the_member(keyword.value):
...
def _mentions(source: str) -> int:
    """How many times the member is read off its enum anywhere in `source`."""
    return sum(_names_the_member(node) for node in ast.walk(ast.parse(source)))
