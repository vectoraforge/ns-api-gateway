...
from nativespeaker.api.auth.devicecheck import (
    ...
    DEVICECHECK_BACKOFF_BASE_SECONDS,
    DEVICECHECK_BACKOFF_MAX_SECONDS,
    DEVICECHECK_HOST,
    QUERY_PATH,
    UPDATE_PATH,
    AppleDeviceCheck,
...
class TestTheAttemptsAreSeparatedInTime:
    ...
    def test_the_configured_gaps_are_the_two_this_floor_was_measured_against(self):
        ...
        gaps = [min(DEVICECHECK_BACKOFF_BASE_SECONDS * 2 ** attempt, DEVICECHECK_BACKOFF_MAX_SECONDS)
                for attempt in range(DEVICECHECK_ATTEMPTS - 1)]
...
class TestAnAbsentCredentialFailsClosed:
    ...
    async def test_a_present_but_unusable_key_is_that_same_absent_state(self, contents, tmp_path):
        """WR-22: an unparsed PEM reached `jwt.encode` and raised past the retry frame onto the
        generic 500 on every claim. CR-09: the last two parse and then fail with `AttributeError`
        and `TypeError`, outside the first except tuple, so they crashlooped the pod from boot."""
